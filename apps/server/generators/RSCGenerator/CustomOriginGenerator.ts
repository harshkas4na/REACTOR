...
import { EventInfo, RSCConfig } from '../../types/ExtDAppAutomationTypes';
...
export class CustomOriginRSCGenerator {
    ...
    private generateEventHandling(config: ReactLogicConfig): string {
        ...
        eventHandling += config.events.map((event: EventInfo, index: number) => `
