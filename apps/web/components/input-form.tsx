...
const CHAINS = {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    const originChain = CHAINS[values.originChain];
    const destChain = CHAINS[values.destinationChain];
    ...
    try {
      const response = await fetch('http://localhost:5000/rsc-monitor', {
        method: 'POST',
        headers: {
                          ...
                          {originChainOptions.map(({ value, label, hasCallbacks }) => (
                            <SelectItem key={value} value={value}>
                              <div className="flex items-center justify-between w-full">
                                <span>{CHAINS[value].icon} {label}</span>
                                {hasCallbacks && (
