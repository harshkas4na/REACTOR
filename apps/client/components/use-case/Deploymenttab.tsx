...
export default function DeploymentTab({ 
  ...
  const [deploymentMode, setDeploymentMode] = useState<Record<string, 'new' | 'existing'>>({
    ...
    destination: 'new',
    reactive: 'new',
    ...Object.fromEntries(helperContracts?.map(helper => [helper.name, 'new']) || [])
  });
...
setConstructorArgs({
  ...
  helpers: Object.fromEntries(
    helperContracts?.filter(helper => helper.abi)
      .map(helper => [helper.name, parseConstructorArgs(helper.abi!)]) || []
  )
});
  ...
  try {
    ...
    setIsDeploying(prev => {
      if (type.startsWith('helper_')) {
        return {
          ...prev,
          helpers: { ...prev.helpers, [type]: true }
        };
      }
    ...
    // Check balance including the deployment fee
    ...
    if (deployedContract) {
      ...
      // Update deployment info with success status
      if (type.startsWith('helper_')) {
        setDeploymentInfo((prev:any) => ({
          ...prev,
          helpers: {
        ...
        setDeployedAddresses((prev:any) => ({ ...prev, [type]: contractAddress }));
    ...
    setIsDeploying(prev => {
      if (type.startsWith('helper_')) {
        return {
          ...prev,
          helpers: { ...prev.helpers, [type]: false }
        };
      }
      ...
      <div className="space-y-4">
          ...
          <div>
            ...
            <Input
              ...
              onChange={(e) => setExistingAddresses(prev => ({
                ...prev,
                helpers: { ...prev.helpers, [type]: e.target.value }
              }))}
        ...
        {address ? (
          ...
          <>
            ...
            {helper.abi && Object.keys(constructorArgs.helpers[helper.name] || {}).length > 0 && (
              <div className="space-y-4">
                <h3 className="text-lg font-medium text-gray-200">Constructor Arguments</h3>
                {Object.entries(constructorArgs.helpers[helper.name] || {}).map(([name, value]) => (
                  <div key={name}>
                    ...
                    <Input
                      ...
                      onChange={(e) => setConstructorArgs(prev => ({
                        ...
                        helpers: {
                          ...prev.helpers,
                          [helper.name]: { ...prev.helpers[helper.name], [name]: e.target.value }
                        }
                      }))}
                      className="mt-1 bg-gray-700 text-gray-200"
      ...
      <div className="space-y-4">
        ...
        {address ? (
          ...
          <>
            ...
            {Object.keys(constructorArgs[type]).length > 0 && (
              <div className="space-y-4">
                <h3 className="text-lg font-medium text-gray-200">Constructor Arguments</h3>
...
<div className="space-y-6">
  ...
  {helperContracts && helperContracts.length > 0 && (
    <>
      <div className="mt-8">
